...
const SectionCalculator = () => {
  ...
  const widthChartData = useMemo(() => {
    if (!results.widthAnalysisData.length) return [];
    return results.widthAnalysisData.map((p, i) => {
      const row = { b: Number(p.b.toFixed(2)), h: Number(p.h.toFixed(3)) };
      results.widthAnalysisExtra.forEach(ex => {
        if (ex.points[i]) row[`m${ex.m}`] = Number(ex.points[i].h.toFixed(3));
      });
      return row;
    });
  }, [results]);
            ...
            <div className="md:col-span-2 bg-white dark:bg-slate-800 p-4 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
              ...
              <div className="h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={widthChartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                    ...
                    <XAxis 
                      ...
                      stroke="#cbd5e1"
                      label={{ value: 'b [m]', position: 'insideBottomRight', offset: -5, fill: '#94a3b8', fontSize: 12 }}
                    />
                    <YAxis 
                      tick={{ fill: '#94a3b8', fontSize: 11 }} 
                      stroke="#cbd5e1"
                      label={{ value: 'h [m]', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 12 }}
                    />
                    <Tooltip 
                    ...
                    <ReferenceLine 
                      ...
                      stroke="#DC2626" 
                      strokeDasharray="5 5" 
                      label={{ value: `h_max=${params.h_total.toFixed(2)}`, fill: '#DC2626', fontSize: 11, position: 'right' }} 
                    />
                    <Line type="monotone" dataKey="h" name={`m=${params.m}`} stroke="#2563EB" strokeWidth={2} dot={false} />
                    {results.widthAnalysisExtra.map((ex, idx) => (
                      <Line 
                        key={ex.m} 
                        type="monotone" 
                        dataKey={`m${ex.m}`} 
